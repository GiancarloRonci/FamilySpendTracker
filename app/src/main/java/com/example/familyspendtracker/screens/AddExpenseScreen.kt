        ...
        Button(
            onClick = {
                ...
                if (selectedPassiveCategoryId == null || selectedWalletId == null) {
                    Toast.makeText(context, "Seleziona categoria e wallet", Toast.LENGTH_SHORT).show()
                    return@Button
                }

                val selectedCategory = categories.find { it.id == selectedPassiveCategoryId }
                val selectedWallet = wallets.find { it.id == selectedWalletId }

                if (selectedCategory == null || selectedWallet == null) {
                    Toast.makeText(context, "Errore nel recupero dati", Toast.LENGTH_SHORT).show()
                    return@Button
                }

                if (parsedAmount > selectedCategory.currentBalance || parsedAmount > selectedWallet.currentBalance) {
                    Toast.makeText(
                        context,
                        "Spesa non consentita: supera il budget disponibile della categoria o del wallet.",
                        Toast.LENGTH_LONG
                    ).show()
                    return@Button
                }
